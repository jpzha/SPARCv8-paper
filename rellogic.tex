...
\begin{proof}
    ...
    in \Def{\ref{def:sim-imp-prim-state}}. So, we can prove 
...
\begin{proof}
    ...
    \begin{itemize}
        \item The well-formed instruction sequence shown as 
            ...
            \Fig{\ref{fig:Selected Inference Rules for Refinement Verification}}. 
            We can finish the proof by doing 
            induction on $\wdcblk{\Cspec}{\relastP}{\relastQ}{\lab{}}{\cblk}$
            and discussing each case respectively.
        ...
        \item We unfold $\wfcdhp{\asmimp}{\Cspec}$ and get that 
            ...
            \implies 
            \semwfseqrel{\Cspec}{\relastP}{\relastQ}{\cblk}$, 
            we can finish the proof by applying this conclusion. 
        ...
        \item We unfold $\wfprim{\Cspec{}}{\asmimp}{\hprimset}$ 
            ...
            third, we can prove it by apply 
...
\begin{proof}
    ...
    \begin{enumerate}[1.]
        \item We introduce a whole program simulation
            defined in \Def{\ref{def:wpsim}}. By unfolding 
            $\semWdPrim{\Cspec}{\asmimp}{\hprimset}$, 
            we get that for all $\lab{} \in \dom(\hprimset), 
            \lgvl$, there exisits $\hprim$, $\args{\val}$, 
            $\relspecpre$, $\relspecpost$, such that: 
            \begin{itemize}
                \small
            ...
            We can prove that if each \sparc{} function has 
            a simulation relation defined in \Def{\ref{def:simfunc}}
            with its corresponding abstract assembly primitive, 
            then we can get that the whole program simulation holds.
            It means that there exists $i \in \Index$, such that: 
            \begin{equation}
                (\code\uplus\asmimp, \state, \pc, \npc)
                \wpsim^{i} 
                ((\code, \hprimset), \hpstate)
                \label{a:wpsim}
            \end{equation}
            The correctness proof is done in the following way. 
            \textit{First}, if the low-level \sparc{} program 
            executes the client code $\code$, we can know 
            that the high-level executes the same instruction, 
            because the client code in low- and high-level is 
            the same. When executing the client code, the low- 
            and high-level program state satisfies the state relation 
            defined in \Sec{\ref{subsec:correctness-primitive}} 
            and it will be preserved after low- and high-level
            program executes the same code as shown below:
            
            \textit{Second}, if the low-level \sparc{} program 
            executes the implementation of the abstract assembly 
            primitive, the high-level executes the abstract assembly 
            primitive and the simulation for function define in 
            \Def{\ref{def:simfunc}} ensures that for each step 
            of low-level execution, the high-level program will 
            execute zero or one step as shown below:



        \item The whole program simulation (\ref{a:wpsim}) can implies the 
            refinement relation (\ref{g3}) between low- and high-level 
            program, which is straightforward, because the 
            whole program requires that when low-level generates 
            an observable event, the high-level can generate 
            the same event. 
    \end{enumerate}
\end{proof}
