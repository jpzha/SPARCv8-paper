...
The Pseudo-SPARCv8 language in this 
contains two part: 
...
We first establish a relation as an {\it invariant} 
...
which means $\prog$ produce no more observable behaviors 
...
The tokens will be consumed when meeting \jmp{} and \call{} 
instructions, so as to avoid infinite loop and recursive 
function. The execution of high-level abstract assembly 
primitive can reset the number of tokens. 
...
\begin{definition}[Simulation for Instruction Sequence]
    ...
    \begin{enumerate}[1.]
		...
		\begin{itemize}
			...
			for any $\state', \pc', \npc'$, \\ if
            $\ptrans{(\state, \pc, \npc)}{(\state', \pc', \npc')}$, then
            there exists $\primcom'$ and $\hpstate'$, such that: 
            \begin{enumerate}[(1)]
        ...
        \item if $\asmimp(\pc) = \retl$ then :
            \begin{itemize}
                ...
                if
                $\multiptrans{2}{(\state, \pc, \npc)}{(\state', \pc', \npc')}$,
                then there exists $\primcom', \hpstate', \word'$, 
                such that: 
                \begin{enumerate}[(1)]
                    \item either $\primcom' = \primcom$, 
                    $\hpstate' = \hpstate$ and $\word' = \word$; 
...
sequences and well-formed internal function. 
...
all the instruction sequences in code heap $\asmimp$ given specifications 
can execute safely.
...
Then we define the simulation for function in 
...
The we give the semantics of well-formed primitive in 
