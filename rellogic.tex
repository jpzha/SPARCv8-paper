...
\begin{proof}
    ...
    \begin{itemize}
        \item If $\comm \in \dom(\code)$, we use the 
            ...
            \end{center} 
            We can know 
            that the high-level executes the same command 
        ...
        \item If $\comm \in \dom(\asmimp)$, we use the 
            ...
            Such simulation relation ensure that for each step 
...
\begin{proof}
    ...
    \end{equation}
    Then, by applying Lemma~\ref{lemma:wpsim-implies-refinement}
    on (\ref{a:wpsim}), it is done. 
\end{proof}
