...
And we also extend the program logic to support refinement 
verification. The extended program logic in our current 
work is designed for verifying the {\it contextual refinement} 
between the implementation of context switch routine 
and $\primsw$ primitive mainly. It will be interesting to 
improve and use our extended program logic to verify 
the correctness of more abstract assembly primitives, 
like thread creating primitive \textsf{spawn}, and 
thread joining primitive \textsf{join}, whose execution 
will add or delete some threads. 
Our current work can only handle
sequential \sparc{} program verification, and do not 
consider interrupts in our machine model. 
Finally, the client code in our contextual 
refinement verification is SPARCv8 code. However, 
many works, like \cite{Xu16cav}, use C program 
as the client code of calling abstract assembly primitive.  
We would like to link the verified inline assembly
with verified C code for whole system
verification in the future. Maybe it's possible to 
follow some complier verification works, 
\eg{\cite{CompCert, CASCompCert}}, to make sure that 
the behaviors of the client C code is equivalent with 
the complied SPARCv8 code, which acts as a client code 
in our work.  
