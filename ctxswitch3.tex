...
The set of abstract assembly primitive contains only 
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        The definition of $\primsw$ restricts that the 
        program counters will point to the correct pointers 
        after execution. And the execution of $\primsw$ 
        only accesses the current and new tasks, and the 
        memory location of $\TaskNew$. 
        Thus, the threads and memory in $\hpstate_r$ 
        ...
        Thus, it's not difficult to prove that the 
        ...
        the current and new tasks, and the 
...
each code block is well-defined in 
...
\begin{proof}
    ...
    $\lab{\textit{sn}}$ to represent \SwitchEntry{}, 
    \regsave{}, \regrestore{}, \SaveUsedWin{} and 
    \SwitchNewTask{} due to space limition below):
    \begin{align}
    ...
    (\ref{a:ctxswitch-wfse}) to (\ref{a:ctxswitch-wfsn}) 
    ...
    Here, we apply \myrule{ABSCSQ} rule in verifying 
    (\ref{a:ctxswitch-wfsn}), when context switch routine 
    returns.  
\end{proof}
...
\begin{proof}
    ...
    (\ref{ctxswitch-hprimset}) and (\ref{a:ctxswitch-Cspec}) 
    respectively, and we need to prove the following hold:
    \begin{itemize}
...
and the additional work includes: 
(1) proving that the specification for context 
