...
in these window can be stored correctly 
...
\begin{itemize}
    ...
    Its execution just restores the previous window 
    ...
    \item 
    The special registers also can't be modified arbitrarily.
    The special registers in SPARCv8 usually act specific 
    roles, for example, \regwim{} marks which window are 
    invalid. If we set the value of \regwim{} shown in 
    \Fig{\ref{fig:Abstraction of Register Windows and Memory}}, 
        ...
        \item 
            ...
            $
                \asrtmodel{(\state, \hpstate, \hprim(\args{\val}), \word)}
                     {(\relspecpre \ \lgvl \sepstar \relastP_r)}, 
                (\relspecpost \ \lgvl \sepstar \relastP_r)w 
                    \Longrightarrow 
                    \INV{(\primdone, \notCare)}
            $,   
            % \\
