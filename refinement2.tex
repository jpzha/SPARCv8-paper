...
The semantics of Pseudo-SPARCv8 language is 
a little different with the realistically modelled 
SPARCv8 program introduced before. We detail these 
differences below: 
\begin{itemize}
    \item The Pseudo-SPARCv8 program should be 
        multi-threaded because we hope to 
        verify the $\primsw$ primitive, 
        whose execution will set a ready thread 
        as the current one. The low-level SPARCv8 
        program state doesn't have thread pool.
        \begin{center}
            ...
            \vspace{-0.5em}
        \end{center} 
    \item We make an abstraction for register window 
        mechanism in SPARCv8. We use 
        \Fig{\ref{fig:Abstraction of Register Windows and Memory}}
        to illustrate this abstraction. 
        ...
        In the Pseudo-SPARCv8 program, we abstract 
        the contexts saved as a list (defined as 
        the HFrmList). This abstraction simplifies 
        ...
        stack in memory at the Pseudo-SPARCv8 level. 
        The low-level program doesn't use this 
        ...
        which is abstracted as $\primsw$ primitive
        in high-leve, needs to save the contents 
        of the register window into memory.  
        
    \item The delay buffer $\DBuf$ and special registers 
        are omitted in Pseudo-SPARCv8 program. The special registers 
        are  usually  used  to  record  the state 
        of the processors and modifying them should be 
        carefully. So, for simplicity, we omit them 
        in the Pseudo-SPARCv8 program. Operating them 
        is hidden in the implementation of the 
        abstract assembly primitives.  
        % So, modifying them should 
        % be carefully and hidden in the implementations 
        % of the abstract assembly primitives.
\end{itemize}
...
% % considering programmers in this level 
% % may define various abstract assembly primitives, 
% % we should just give the type of the abstract 
% % assembly primitive, which is like an interface and 
% % its implementation is remained for users to complete. 
% % The abstract 
% % assembly primitive should be like an interface, whose 
% % implementations are remained to user to complete. 
% {\it \textbf{Third}}, 
% % the programmers in the high-level 
% % don't need to know the existance of delayed buffer $\DBuf$.
% delay buffer $\DBuf$ is not needed in high-level program state.  
% % introduced in \Sec{\ref{sec:modeling}}. 
...
% Readers who want to achieve more details of the high-level Pseudo 
% SPARCv8 program can see the Appendix \ref{appendix:more-about-high-level-insExec}. 
\begin{figure*}[!t]
    \centering
    \vspace{-2em}
    \[
