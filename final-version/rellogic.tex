...
%     $$
% }
The execution of the $\primsw{}$ primitive takes no arguments
...
\begin{definition}[Well-defined Specification]
    ...
    \begin{enumerate}[1.]
        \item for any {\em $\args{\val}$}, 
            ...
            (let {\em $\lab{} = \hpstate'.\hthrdlocalst.\hRstate.\hRfile
                        (\reg{15})$}):
            \begin{itemize}
         ...
         \item for any $\lgvl$, there exists $\args{\val}$,
         	{\em $\relspecpre \ \lgvl \Longrightarrow
         	 	\safePrimAst{\hprim(\args{\val})} \sepstar \atrue$},
         	and {\em $\relspecpost \ \lgvl \Longrightarrow
    ...
    \begin{enumerate}[1.]
        ...
        \item if {\em $\asmimp(\pc) = \jmp \; \aexp$} then:
            \begin{itemize}
                ...
                \item for any {\em $\state', \pc', \npc'$}, \\ if
                {\em $\multilptrans{\asmimp}{2}{(\state, \pc, \npc)}{(\state', \pc', \npc')}$},
                then there exists {\em $\relspecpre, \relspecpost, \lgvl,
            ...
            \begin{itemize}
                ...
                \begin{enumerate}[(1)]
                    ...
                    then \\
                    {\em $\simInsSeq{\relastQ}{\Cspec}{(\asmimp, \state', \pc\!+\!8, \npc\!+\!12)}
...
we define the semantics for well-formed instruction
sequences and well-formed code heap in 
                ...
                \item for any $\state'$, {\em $\pc'$}, {\em $\npc'$}, \\
                    ...
                    \begin{enumerate}
                        ...
                        \item if $k = 0$, then there exists
                        	$\word'$, such that:
                        	(let {\em $\lab{} = \state'.\Rstate.\RFile(\reg{15})$}) \\
                            \hspace*{2em}
                            $\asrtmodel{(\state', \hpstate', \primcom', \word')}
                                {\relastQ}$,
                            $\primcom' = \primdone$, \\
