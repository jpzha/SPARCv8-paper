...
%, which is the expansion of our conference paper.
...
\begin{center}
    ...
    (a) Reigster windows and stack in memory. 
    (b) High-level frame list.}
...
The executions of client code are require to
...
\begin{itemize}
    ...
    Therefore, we forbid the client
...
The code (\TYPE{HCode}) $\HMd$ includes
the code heap $\code$
...
%                        \hRfile'(\spreg) = (\block_0, 0)
    ...
    (a) High-level Program Transisition. 
    (b) High-level Thread Local Transisition.
    (c) High-level Instruction Transistion.} 
...
in low-level are both \sparc{} code heap. 
There is no need to define their linking 
in semantics.
...
is ``abstracted" as a thread pool
...
\]
The client code $\code$ is \sparc{} code. 
Therefore,
...
\begin{center}
    \[
        \begin{array}{rcl}
            \text{(RelAsrt)} \quad \relastP, \relastQ & \ \define \ &
            \astP \sepline
...
\[
    ...
    \begin{array}{lccl}
        ...
        \TYPE{valList} \rightarrow \TYPE{RelAsrt} \\
    ...
    \subfigure
    {
        \begin{minipage}{1\textwidth}
            $\boxed{\wdcblk{\Cspec}{\relastP}{\relastQ}
                {\lab{}}{\cblk}}$ \quad \ \ 
            \text{(Well-formed Instruction Sequences)}
            \[
                \begin{array}{c}
                    \infer[(\myrule{SEQ})]
                    {
...
% conjunction $\sepstar$.
The \myrule{ABSCSQ} rule allows us to execute the high-level
...
\begin{definition}[Well-defined Specification]
    ...
    \begin{enumerate}[1.]
        \item for any {\em $\args{\val}$}, 
            ...
            \begin{itemize}
                \item {\em $\hpstate'.\hthrdlocalst.\pc = \lab{}\!+\!8$},
                    {\em $\hpstate'.\hthrdlocalst.\npc = \lab{}\!+\!12$};
                \item there exist $\hpstate''$, $\hpstate_r'$, such that
        ...
        \item
            ...
            there exist $\lgvl, \relastP_r$ and $\word$,
            such that \\
            {\em $
...
Second, the abstract assembly primitive should be
...
% from the safe execution of the primitive $\primsw$.
% and we
% can write them in the precondition of the specification
% of the context switch routine.
% Otherwise,
% the execution of primtive $\primsw$ will not be safe.
% We in turn know that the memory location $\TaskNew$
...
% so as to avoid infinite loops and recursive
...
% which means that the relational program logic can
% imply the contextual refinement between the implementation
% $\asmimp$ and abstract assembly primitive set $\hprimset$. 
% We give soundness proof below.
...
% relations 
% between the SPARCv8 implementations and abstract assembly primitives.
...
%     ensures that the client code does not include
...
\begin{proof}
    ...
    %     \tag{g3} \label{g3}
    % \end{equation}
    % Here, $\code$ is the client code.
    % $\state$ and $\hpstate$ represent the initial states
    % of low- and high-level programs respectively.
    % By Lemma~\ref{lemma:function sim implies whole program sim},
    % we get there exisits $i \in \Index$, such that:
    % \begin{equation}
    %     (\code\uplus\asmimp, \state, \pc, \npc)
    %     \wpsim^{i}
