...
features of \sparc{}, 
{\color{blue} and the relational program logic for  
refinement reasoning is introduced in  
...
Lemma~\ref{lemma:RFileSplitExDelay}, and 
we present the proof of this lemma below.
\begin{proof}
	``$\Longrightarrow$": 
	...
	\end{align}
	We unfold (\ref{a:asthold}) according to the definition 
	of $(\notCare \sepstar \notCare)$ and get there exists 
	$\Mem_1$, $\Mem_2$, $\RFile_1'$ and $\RFile_2'$ such that:
	\begin{align}
		& \Mem = \Mem_1\cup\Mem_2, \, \Mem_1\perp\Mem_2 
		\label{a:memsplit} \\
		& \RFile' = \RFile_1'\cup\RFile_2', \, \RFile_1'\perp\RFile_2'
		\label{a:regsplit} \\
	...
	where $\RFile_1\perp\RFile_2$ and 
	$\RFile = \RFile_1\cup\RFile_2$, such that
	\begin{align}
	...
	$\asrtmodel{(\Mem, (\RFile, \Wstack), \DBuf)}
		{(\nxtAst{\astP})\sepstar(\nxtAst{\astQ})}$ holds, 
	and finish the proof of this case.

	``$\Longleftarrow$": if 
	$\asrtmodel{(\Mem,(\RFile,\Wstack),\DBuf)}{(\nxtAst{\astP})\sepstar(\nxtAst{\astQ})}$,
	then $\asrtmodel{(\Mem, (\RFile, \Wstack), \DBuf)}
	{\nxtAst{(\astP\sepstar\astQ)}}$. According to the 
	definition of $\nxtAst{(\notCare)}$ and $(\notCare \sepstar \notCare)$, 
	we get that there exists $\Mem_1$, $\Mem_2$, $\RFile_1'$, 
	$\RFile_2'$, $\RFile_1$, $\RFile_2$ and $\DBuf'$, such that:
\end{proof}
