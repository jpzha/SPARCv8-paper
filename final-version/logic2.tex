...
that does not support refinement verification to
...
(\text{Asrt}) in Fig.~\ref{fig:Syntax of Assertions},
...
\begin{proof}
	...
	(A1) $\Dstep{(\RFile', \DBuf')}{(\RFile, \DBuf)}$; 
	(A2) $\RFile' = \RFile_1'\uplus\RFile_2'$; 
	(A3) $\Mem = \Mem_1\uplus\Mem_2$;
	(A4) $\asrtmodel{(\Mem_1, (\RFile_1', \Wstack), \DBuf')}{\astP}$; 
	(A5) $\asrtmodel{(\Mem_2, (\RFile_2', \Wstack), \DBuf')}{\astQ}$. 
	% \begin{align}
	% 	& \Dstep{(\RFile', \DBuf')}{(\RFile, \DBuf)} \\
	% 	& \RFile' = \RFile_1'\uplus\RFile_2'. 
	% 		\label{a:dstep1-regsplit} \\
	% 	\label{a:regsplit} \\
	...
	% $\asrtmodel{(\Mem, (\RFile, \Wstack), \DBuf)}
	% 	 {(\nxtAst{\astP})\sepstar(\nxtAst{\astQ})}$ holds 
	% by (\ref{a:regsplit}) (\ref{a:memsplit}) (\ref{a:pholds})
	% (\ref{a:qholds}) (\ref{a:dstep1}) (\ref{a:dstep2}).
	
	Thus, 
	we get $\asrtmodel{(\Mem_1, (\RFile_1, \Wstack), \DBuf)}{\nxtAst{\astP}}$; 
	and $\asrtmodel{(\Mem_2, (\RFile_2, \Wstack), \DBuf)}{\nxtAst{\astQ}}$.
	...
	where $\RFile = \RFile_1\uplus\RFile_2$, such that:
	(B1) $\Dstep{(\RFile_1', \DBuf_1')}{(\RFile_1, \DBuf)}$, 
			$\Dstep{(\RFile_2', \DBuf_2')}{(\RFile_2, \DBuf)}$;
	(B2) $\asrtmodel{(\Mem_1, (\RFile_1', \Wstack), \DBuf_1')}{\astP}$; 
	(B3) $\asrtmodel{(\Mem_2, (\RFile_2', \Wstack), \DBuf_2')}{\astQ}$. 
	% \begin{align}
	...
	$\DBuf' = \DBuf_1' = \DBuf_2'$. 
	By applying Lemma~\ref{lemma:RFileSplitExDelay} 
	on (B1), we get there exists 
	$\RFile'$, where $\RFile' = \RFile_1'\uplus\RFile_2'$,  
	such that $\Dstep{(\RFile', \DBuf')}{(\RFile, \DBuf)}$. 
	% \begin{equation}
	% 	\Dstep{(\RFile', \DBuf')}{(\RFile, \DBuf)}
	% 	\label{a:dstep2}	
	% \end{equation}
...
\[
	...
	\begin{array}{lccllccl}
		...
		\multicolumn{4}{l}
		{
			...
			\text{(pAsrt)} \ \
			\specpre,\specpost \in
			\text{valList} \rightarrow \TYPE{Asrt}
		}
		% \specpre,\specpost &
...
The top rule \RULE{CDHP} verifies the code
...
The former makes an ``intra-function" control transfer, 
...
The \myrule{BE} rule checks the ``current" value of the register $\regz$
