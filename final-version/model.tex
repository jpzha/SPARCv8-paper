...
$\prog$ includes the code heap (\TYPE{CodeHeap})
...
and the address expression (\TYPE{AddrExp}) $\aexp$,
...
Commands (\TYPE{Comm}) in \sparc{} are classified into two categories:
(1) the simple instructions (\TYPE{SimpIns})
...
\begin{figure*}[!t]
	\centering
	\includegraphics[width=9cm]{window}
