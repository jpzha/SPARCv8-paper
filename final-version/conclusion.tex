...
(but it does not support function call and return).
...
\etal{Hou}~\cite{sparcv8-formalization-Isabelle} modelled the SPARCv8 ISA
...
\etal{Wang}~\cite{sparc-formalization} formalized its semantics in Coq.
...
However neither \etal{Wang} nor we validate the
...
\etal{Ni}~\cite{ctxm} verified a context switch module of 19 lines
...
implemented in \sparc{} contextually refines the $\primsw$
