...
However, the OS kernal is implemented as 
...
state satisfies the precondition required, is insufficient.
\etal{Xu}~\cite{Xu16cav} propose a program logic for 
verifying the correctness of OS kernal implementated in 
...
we use $\Clang$, $\hprimset$, and $\asmimp$ to represent 
the C language program, the set of abstract assembly 
primitive $\hprimset$, and the implementations of 
abstract assembly primtives: 
\[
    \forall \, \Clang. \, 
    \Clang[\hprimset] \refine \Clang[\asmimp]
\]
However, if we use C program as a client code to 
call inline assembly code, we define define the 
...
\begin{center}
    $
        ...
        \end{array}
    $
    \figurecaption{Idea to achieve fully certified
        OS kernal for aerospace crafts}
    \label{fig:idea to fully certified OS kernal}
\end{center}
...
We use \Fig{\ref{fig:idea to fully certified OS kernal}} 
to illustrate our idea. We introduce this 
figure from top to down. The source program of OS kernal, 
which is represented as $\Clang[\hprimset]$, 
is implementated as C language 
with a set of abstract assembly primtives. 
The source program executes 
under C language semantics (represented as 
$\semantics{\notCare}^{\textsf{C}}$). The compiler 
(\textit{Comp}) translates the C program $\Clang$ to 
SPARCv8 code $\code$. 
As {\color{blue} \textcircled{1}} shown, 
we hope the compilation makes sure the 
refinement relation between $\code[\hprimset]$, which 
executes under Pseudo-SPARCv8 semantics represented as 
...
and its semantics will be defined in the following. 
Finally, we can get 
$\semantics{\Clang[\hprimset]}^{\textsf{C}}
\refine
\semantics{\code[\asmimp]}^{\textsf{SPARCv8}}$. 
