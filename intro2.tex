...
to callee (\texttt{ChangY}). Here, $\spreg$ register, 
which is an alias of $\reg{14}$, acts as the stack pointer. 
...
for some convenience in program verification. 
% Then, considering some works, \eg{\cite{Xu16cav, Feng08pldi}},
...
%	and can check realistic \sparc{} code easily and conveniently.
    ...
    relation, which can be established 
    between the execution of each SPARCv8 function's 
    implementation and its abstract assembly primitive. 
...
in Sec.~\ref{sec:ctxswitch}. The extended program logic to 
