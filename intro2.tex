...
\begin{itemize}
    ...
    of registers such as $\regwim$ used to justify 
    whether a register window is valid does not take 
    effect immediately. That is, ``they may take until 
...
the \texttt{CALLER} uses instruction \csave{} 
(at line 3) to save its context 
(\localRN{} and \inRN{} registers) and 
let the \outRN{} registers become the \inRN{} registers  
by window rotation. So, the $\ireg{0}$ register 
that is assessed in \texttt{ChangY} at line 6 is 
the same register as the $\oreg{0}$ modified by 
